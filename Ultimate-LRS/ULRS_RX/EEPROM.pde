...
unsigned int read_eeprom_uint(int address) {
    return (EEPROM.read(eeprom_start_address + address) * 256)
            + EEPROM.read(eeprom_start_address + address + 1);
}
...
void load_failsafe_values() {
    for (int i = 0; i < 8; i++) {
        Servo_Buffer[i] = (EEPROM.read(eeprom_start_address + 100 + (2 * i))
                * 256) + EEPROM.read(eeprom_start_address + 101 + (2 * i));
    }
}

void save_failsafe_values(void) {
    for (int i = 0; i < 8; i++) {
        EEPROM.write(eeprom_start_address + 100 + (2 * i),
                Servo_Buffer[i] / 256);
        EEPROM.write(eeprom_start_address + 101 + (2 * i),
                Servo_Buffer[i] % 256);
    }
}
...
void eeprom_check(void) {
    ...
    if (Serial.available() > 2) {
        if ((Serial.read() == 'R') && (Serial.read() == '4')
                && (Serial.read() == 'T')) {
            while (1) {
                if (Serial.available() > 2) {
                    temp1 = Serial.read();
